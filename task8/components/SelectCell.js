...
import { createLine } from "./canvasComponents.js";
...
export class SelectCell {
	...
	constructor(sheetIndex, zoom = 1) {
		...
		this.dpr = handleDynamicDPI(this.canvas, this.ctx); // Handle device pixel ratio
	...
	_drawSelection(e) {
		...
		// Calculate selected cell row and column
		const rowNum = Math.floor(e.offsetY / (15 * this.zoom));
		const colNum = Math.floor(e.offsetX / (50 * this.zoom));
